...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine elegir_sitio(nrows,ncols,posx,posy,iniseed,probpos,matriz,average)
	
	!This subroutine gives a random point in the square matrix of LxL
	integer,intent(in)::nrows,ncols,iniseed,average
	integer,intent(out)::posx,posy
	real,intent(in)::probpos
	integer,intent(in)::matriz(nrows,ncols)
	integer::i
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine CalcDeltaEnergy(nrows,ncols,posx,posy,matriz,alpha,Delta_E,ip,im,iniseed,newposx,newposy)
	!Calculate Delta for a given point (posx,posy)
	!with periodic boundary conditions
	integer,intent(in)::nrows,ncols,iniseed
	integer,intent(in)::posx,posy,matriz(nrows,ncols),ip(nrows),im(nrows)
	integer,intent(inout)::newposx,newposy
	real,intent(in)::alpha(3,3)
	real,intent(out)::Delta_E
	...
	call CalcEnergy(nrows,ncols,newposx,newposy,matriz,alpha,Delta_E2,ip,im) 
	...
	Delta_E3 = Delta_E3*matriz(newposx,newposy) 
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine CellInteraction(matriz,ncols,nrows,ip,im,npos,averIndex, &
	...
	integer(dp2),intent(out)::interPos,interNeg,interPN
	integer(dp2)::iPos,iNeg,iPN
	integer::index1,index2
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine HowManySpins(matriz,ncols,nrows,ip,im,npp,index1,index2)
	!This subroutine counts the number of positive spins around a fixed point (index1,index2)
	!The result is stored in the variable npp
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine CellInteractionV2(matriz,ncols,nrows,ip,im,npos,averIndex, &
	...
	do index1=1,nrows
		do index2=1,ncols
			call HowManySpins (matriz,ncols,nrows,ip,im,npp,index1,index2)
			!Is the cell negative or positive?
			if (matriz(index1,index2) == 1) then
				!It is positive. 
				!Are the neighbours positive or negative?
				iPos = iPos + npp
				iPN = iPN + 4 - npp
			else
				!It is negative. Are the neighbours also negative?
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine WriteInteractions(IDfile,interPos,interNeg,interPN,npos,nrows,ncols,npoints)

integer,intent(in)::ncols,nrows,npoints,IDfile
real, parameter::pi = 3.1415927
real,intent(in)::npos
integer(dp2),intent(in)::interPos(npoints),interNeg(npoints),interPN(npoints)
...
do j = 1,npoints
	...
	!Write to the file				
	write(IDfile,*) j,interPos(j),interNeg(j),interPN(j), rIdeal, rMeasured, rho
