...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine CalcDeltaEnergy(nrows,ncols,posx,posy,matriz,Delta_E,ip,im,alpha,currentField)
...
subroutine CalculationsINequilibrium(nrows,ncols,matriz,Delta_E,EnergyAver,EnergyAver2, &
									MagnetizationAver,MagnetizationAver2,MagnetizationAver4,ip,im,alpha,currentField)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine Write2File ( promediateCounter,MagnetizationAver,MagnetizationAver2,MagnetizationAver4,EnergyAver,EnergyAver2,fieldValues,NumofTempPoints,average,currentTemp,folderName,FileName )
	!Write the important values in a file
	integer,intent(in)::promediateCounter,NumofTempPoints,average
	real::currentTemp,beta,currentField,fieldValues(NumofTempPoints)
	...
	character(len=100),intent(in)::FileName,folderName

	integer::i
	open(unit=1,file=trim(folderName)//'/'//trim(FileName),access="append",action="write")
	do i = 1,NumofTempPoints
		...
		!end if
		write(1,*) currentTemp,',',currentField,",",MagnetizationAver(i)/real(promediateCounter*average),",", MagnetizationAver2(i)/real(promediateCounter*average),",",&
					 Suceptibility ,",", Cumulant,',',EnergyAver(i)/real(promediateCounter*average),",", EnergyAver2(i)/real(promediateCounter*average),",", C_v
	end do
	close(1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine New_Field(indexTemp,currentTemp,Tmax,TempInterval)
