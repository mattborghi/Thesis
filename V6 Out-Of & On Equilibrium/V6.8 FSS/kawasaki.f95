...
implicit none
	...
	real::Delta_E
	real::rvec(1)
	real(dp),allocatable::Magnetization(:),Energy(:),antiMagnetization(:),AbsMagnetization(:)
	real(dp)::MagnetizationINI=0.0,EnergyINI=0.0,antiMagnetizationINI=0.0,AbsMagnetizationINI=0.0
	...
	!---------------------------------------
	real::currentTemp=2.3,currentExtMagnet=0.0,alpha13=1,inimagnet=0.04
	integer::nrows=20
	...
	allocate(Energy(MCS))
	allocate(antiMagnetization(MCS))
	allocate(AbsMagnetization(MCS))
	...
	call SetToZeroVariables(Magnetization,Energy,antiMagnetization,AbsMagnetization,MCS)
	MagnetizationINI = 0.0
	AbsMagnetizationINI = 0.0
	EnergyINI = 0.0
	...
	do k = 1 , average
		...
		call CalcQuantities(nrows,ncols,matriz,Delta_E,EnergyINI,MagnetizationINI,antiMagnetizationINI,AbsMagnetizationINI,ip,im,alpha,k,currentExtMagnet)
		...
		do i=1,MCS
			...
			call CalcQuantities(nrows,ncols,matriz,Delta_E,Energy(i),Magnetization(i),antiMagnetization(i),AbsMagnetization(i),ip,im,alpha,k,currentExtMagnet)
		...
		write(1,*) "average MCS Magnetization AbsMagnetization antiMagnetization Energy "
		call Write2File(Magnetization,AbsMagnetization,antiMagnetization,Energy,MagnetizationINI,AbsMagnetizationINI,antiMagnetizationINI,EnergyINI,MCS,k)
		close(1)	
