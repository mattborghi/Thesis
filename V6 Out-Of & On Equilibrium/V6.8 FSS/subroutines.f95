...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!1
subroutine SetToZeroVariables(MagnetizationAver,EnergyAver,antiMagnetization,absMagnetizationAver,length)

	integer,intent(in)::length
	real(dp2),intent(out)::MagnetizationAver(length),absMagnetizationAver(length)
	real(dp2),intent(out)::antiMagnetization(length),EnergyAver(length)

	MagnetizationAver(:) = 0.0
	absMagnetizationAver(:) = 0.0
	EnergyAver(:) = 0.0
...
subroutine CalcQuantities(nrows,ncols,matriz,Delta_E,EnergyAver,MagnetizationAver,antiMagnetization,absMagnetization,ip,im,alpha,averageIndex,currentField)
	...
	real,intent(in)::alpha(3,3),currentField
	real(dp2),intent(inout)::EnergyAver
	real(dp2),intent(inout)::MagnetizationAver,absMagnetization
	real(dp2),intent(inout)::antiMagnetization
	integer::i,j
	integer::dim
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine Write2File ( Magnetization,absMagnetization,antiMagnetization,Energy,MagnetizationINI,absMagnetizationINI,antiMagnetizationINI,EnergyINI,length,average )
	!Write the important values in a file
	integer,intent(in)::length,average
	real(dp2),intent(in)::Magnetization(length), absMagnetization(length)
	real(dp2),intent(in)::antiMagnetization(length),Energy(length)
	real(dp2)::MagnetizationINI,absMagnetizationINI,EnergyINI,antiMagnetizationINI
	integer::j

	write(1,'(I6,x,I6,x,F16.6,x,F16.6,x,F16.6,x,F16.6)') average,0+1,(MagnetizationINI/average),absMagnetizationINI/average,antiMagnetizationINI/average,EnergyINI/average
	do j = 1,length		
		write(1,'(I6,x,I6,x,F16.6,x,F16.6,x,F16.6,x,F16.6)') average,j+1,(Magnetization(j)/average),absMagnetization(j)/average,antiMagnetization(j)/average,Energy(j)/average
	end do
