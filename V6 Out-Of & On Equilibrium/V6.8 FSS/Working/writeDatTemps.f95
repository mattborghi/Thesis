...
implicit none
	...
	!---------------------------------------
	real::currentExtMagnet=0.0,alpha13=1,inimagnet=1.0
	integer::nrows=256
	real::currentTemp

	real,allocatable::temps(:) !19
	real::TempMin=1.92,TempMax=2.92,step=0.1
	...
	!end if
	call ini_matriz(matriz,nrows,ncols,nneg,iniseed,1)
	do t = 1,size(temps)
	print *, 'Current Temperature: ',temps(t)
	currentTemp = temps(t)
	...
	!open (UNIT=1, FILE=trim(folderName)//'/'//trim(MagFile),ACTION='write',STATUS='replace',form='unformatted')
		!write(1,*) "MCS Magnetization AbsMagnetization antiMagnetization Energy "
	!close(1)
