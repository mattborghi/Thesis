...
implicit none
	...
	integer,parameter::L=20,T_max = 500,promedio=1
	...
	real::T_inicial
	character(len=25)::filename='magnetization20L.dat',filenames = 'energia20L.dat'
	dim = L*L
	T_inicial=0.9
	Temp = T_inicial
	...
	close(1)
	open (UNIT=33, FILE=filenames,ACTION='write',STATUS='replace')
	close(33)
	...
	do j = 1,T_max !T_max no es una temperatura sino un parámetro que mide el numero de temperaturas intermedias que se miden
		...
		call funciones_especiales(dim,prom,prom_energia,prom_energia2,prom_M2,Temp,suceptibility,cap_calorifica)
		call escritura(filename,filenames,dim,Temp,prom,prom_M2,prom_M4, &
