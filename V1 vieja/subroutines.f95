...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine funciones_especiales(dim,prom,prom_energia,prom_energia2,prom_M2,Temp,suceptibility,cap_calorifica)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine escritura (filename,filenames,dim,Temp,prom_magnet,prom_M2,prom_M4, &
						prom_energia,suceptibility,cap_calorifica,prom_energia2)

	character(len=20),intent(in)::filename,filenames
	integer,intent(in)::dim
	real,intent(in)::prom_M2,prom_M4,prom_magnet,Temp,suceptibility,prom_energia,cap_calorifica,prom_energia2

	real:: cumulant
	cumulant = 1 - ( prom_M4/dim / ( 3*( (prom_M2/dim)**2) ) )

	open (UNIT=1, FILE=filename,ACTION='write',POSITION='append')
		write(1,*) Temp,prom_magnet/dim,suceptibility/dim,cumulant,prom_M2/dim,prom_M4/dim
	close(1)

	open (UNIT=33, FILE=filenames,ACTION='write',POSITION='append')
		write(33,*) Temp,prom_energia/dim,cap_calorifica/dim,prom_energia/dim,prom_energia2/dim
	close(33)
